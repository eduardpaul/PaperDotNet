...
| **8e Tasks, calendar, approvals** | My tasks (assigned, due this week, overdue, all; grouped by due date; quick add to a remembered list; one-click complete); task tabs: checklist, related (subtasks, "blocked by", documents), repeat; calendar month and agenda views with events and due tasks, new events on a day (09:00 in the user's time zone), popovers with "open series" and "skip this one", a repeat editor (RRULE) in the user's time zone, .ics import; the end of an event moves with its start; approvals page (waiting, approved, rejected) with comments | ✅ |
| **8f Collaboration and notes** | Activity tab on every item: comments with replies, @mention suggestions (the mentioned people are sent with the comment and highlighted), editing and deleting your own comments, and the item's history (recorded in the background, so a new item can show none for a moment); a Follow button in the item header (immediate notifications); notes get a Write/Preview Markdown editor (GFM, no raw HTML) where `[[Title#heading\|alias]]` links open the linked note, and a Links tab with outgoing links ("not written yet" when no note has the title) and backlinks | ✅ |
...
| 8i List settings | General (name, folders, versioning), columns through content types, views, document settings (duplicates, OCR), permissions (inheritance, grants) | planned |
