...
| PLT-17 | User preferences | As a **Member**, I want to set my language, time zone, date and number formats, theme and default document languages, so that dates, reminders, digests and OCR defaults fit me and a future UI can follow them | Core | P7 | PM §10 |
| PLT-18 | Tenant defaults | As an **Admin**, I want tenant-wide defaults (language, time zone, formats, default OCR languages for new libraries) that user preferences override, so that new users and libraries start with the right settings | Core | P7 | PM §10 |
...
| IAM-09 | Sharing links | As a **Member**, I want links with expiry, password and view/download/edit rights that I can revoke, so that I can share with people outside the organization | Core | P6 | #0015 |
| IAM-10 | File request links | As a **Member**, I want an upload-only link into a folder, so that clients can send me documents without seeing anything else | Core | P6 | #0015 |
| IAM-11 | Guest users | As an **Owner**, I want to invite external guests who only see what is shared with them, so that partners can work with us safely | Ext | P6 | #0015 |
| IAM-12 | Sharing policies | As an **Admin**, I want to allow or deny anonymous links, cap expiry and restrict guest domains, so that sharing follows company rules | Core | P6 | #0015 |
...
| IAM-14 | Account lifecycle | As an **Admin**, I want to update, deactivate, reactivate and delete users, reset their passwords, rename and delete groups and roles, and remove role assignments; and as a **Member**, I want to change my password, so that accounts stay correct over time (also needed after a Papermerge import, whose passwords can't be carried over) | MVP | P7 | PM §8 |
| IAM-15 | Reverse-proxy authentication | As an **Operator** behind Authelia, Authentik or oauth2-proxy, I want to trust user, group, name and email headers from configured proxies only (off by default), so that users sign in once at the proxy | Ext | P7 | PM §9 |
...
| LST-19 | Fields on folders | As an **Owner**, I want folders to carry fields (at least keywords and terms) through a folder content type, so that I can tag and classify folders like documents | Core | P7 | PM §4 |
...
| DOC-16 | Group inboxes | As a **Member** of a group, I want a shared inbox for the group (for example the scanner of the accounting team), so that incoming documents land where the whole team can process them | Core | P7 | PM §3 |
| DOC-17 | Language per file | As a **Member**, I want to set and keep the language of a single file, overriding the library's languages, so that OCR and search stemming are right for documents in another language | Core | P7 | PM §1 |
...
| API-13 | Client CLI | As a **Member**, I want a command-line client (built on the C# SDK) to upload a local folder with its structure, download and search, so that I can bring in and script bulk work without a UI | Ext | P7 | PM §12 |
...
| **5b Automation** | Automation module ([ADR-0018](adr/0018-automation-workflowcore.md), [ADR-0019](adr/0019-workflows-on-wolverine.md), [ADR-0024](adr/0024-one-automation-model.md), guide [automation.md](automation.md)). One model: automations per workspace (`…/automations`, versioned) with a trigger (`manual`, `itemAdded`/`itemUpdated`/`itemDeleted`/`itemRestored` or an extension trigger; list, content type, changed fields), an OData condition checked when the trigger fires, and steps: action, approval (assignees, due date, escalation), delay, condition (approval outcome or OData). Runs start through the outbox (one per automation and event), resume through durable messages, are leased to one server at a time and recovered when stuck ([ADR-0025](adr/0025-reliable-runs-on-several-servers.md)), keep a log and can be cancelled; chains stop at depth 3; finished runs are deleted after 30 days. Manual automations start on an item (`POST …/items/{id}/automations`); `GET /v1.0/me/approvals` and decisions. EVT-09 `AddAutomationAction`/`AddAutomationTrigger` for extensions (`IAutomationTriggers`). DOC-14 path templates: action `item.file` files and renames items from tokens (`{created:yyyy}/{counterparty}`). Built-in actions `item.update`, `item.file`, `task.create`, `notify`. Automations travel with workspace templates | ✅ |
| 5c Sharing | IAM-08 internal sharing, IAM-09 links, IAM-10 file requests, IAM-11 guests, IAM-12 policies | moved to P6 |
| **5d Collaboration & sync API** | Guide [collaboration-and-sync.md](collaboration-and-sync.md), [ADR-0020](adr/0020-sync-api.md). LST-17 Collaboration module (on the SDK): comments with replies and @mentions (notified when they can read the item, searchable) at `…/items/{id}/comments`, activity timeline at `…/items/{id}/activity` (item changes, comments, approvals, `IItemActivity` for extensions). API-05 `GET …/lists/{id}/items/delta`: full sync then changes and `@removed` deletions via `deltaLink`, 410 `resyncRequired` after permission changes or expiry; change log written in the lists engine. API-06 `/v1.0/changeSubscriptions`: list or item, change types, validation handshake, signed POSTs with ids only, renew up to 30 days. API-04 `POST /v1.0/$batch`: up to 20 requests, `dependsOn`, per-request status | ✅ |
| 5e Channels | NTF-04 email, ntfy, Gotify; NTF-06 channels from extensions | deferred (backlog) |
| **5f Smart folders & taxonomy** | Guide [smart-folders-and-taxonomy.md](smart-folders-and-taxonomy.md), [ADR-0022](adr/0022-smart-folders.md). TAX-08 `/v1.0/smartFolders` (personal or shared per workspace): rules on lists, list kinds, content types, terms (with children) and OData filters across all lists, items merged newest first; relative aliases `@me`, `@today`, `@next7Days`, … in every item filter. TAX-09 drop to classify (`POST …/items`: terms, `eq` conditions and sub-folder values applied through the item pipeline; `DELETE` reverses). TAX-10 `groupBy` virtual sub-folders with counts (`…/groups?path=`), by year/month for dates. TAX-05 popular keywords (usage from the search index) and promotion into term sets (same id, or merge). TAX-11 SharePoint CSV import and `AddTermSet` for extensions. Shared smart folders travel with workspace templates | ✅ |
| 5g Documents & storage | DOC-05, DOC-06 page operations (done in 7f); DOC-15 S3 | S3 in the backlog |
...
| 5i Identity & limits | IAM-04 external OIDC, PLT-06 quotas | deferred (backlog) |

## Phase 6 status

Phase 5's remaining slices (5e, 5g, 5i) are deferred to the backlog; phase 6 starts with search.

| Slice | Features | Status |
|---|---|---|
| **6a Semantic & hybrid search** | Guide [search.md](search.md), [ADR-0027](adr/0027-semantic-and-hybrid-search.md). Passages of about 1,200 characters with page numbers (`search.passages`, own full-text index); Documents supplies page texts (`ItemSearchContent.Pages`). SRC-07 embeddings from any OpenAI-compatible endpoint (`AI:Embeddings`, building block `PaperDotNet.AI` on Microsoft.Extensions.AI; off by default), computed by the job `search.embeddings` (unchanged text is not embedded again; a model change embeds again), searched by an in-memory index per tenant and server (same on SQLite and PostgreSQL), trimmed in SQL. SRC-08 `mode=keyword\|semantic\|hybrid` on `/v1.0/search` and the MCP `search` tool; hybrid fuses the best 200 documents of each side with reciprocal rank fusion; default hybrid when configured. SRC-09 hits carry `page`, a snippet from the best passage (keyword rank, else similarity) and `matchedBy` | ✅ |
| 6b AI | AI-01…06 | planned |
...
| **7d Papermerge import** | Guide [papermerge-import.md](papermerge-import.md), [ADR-0029](adr/0029-papermerge-import.md). PLT-15 `paperdotnet import-papermerge --db --media --tenant --user` (or `--output`): converter `src/Tools/PaperDotNet.Import.Papermerge` with its own read-only EF model of Papermerge 3.6 (Alembic versions pinned), one workspace with a folder per owner (unique permissions; inbox as sub-folder), sharing as grants, document types → content types with custom fields, tags → terms, all versions with page texts (no OCR), original stamps; users created without passwords. Package format additions for everyone: all file versions, page texts, `created`/`modified` stamps (`AuditOverrides`) and item permissions | ✅ |
| **7e Accounts & preferences** | Guide [accounts-and-preferences.md](accounts-and-preferences.md), [ADR-0030](adr/0030-account-lifecycle-and-preferences.md). IAM-14 `PATCH`/`DELETE /v1.0/users/{id}` (disable ends sessions and tokens; delete anonymizes and frees the user name, `PrincipalDeleted` removes grants and workspace memberships), admin reset `POST /v1.0/users/{id}/password`, `POST /v1.0/me/password`, `PATCH`/`DELETE` groups and roles, `DELETE …/roles/{id}/assignments/{id}`; never without an enabled administrator; password changes end sign-in sessions (security stamp). PLT-17 `/v1.0/me/preferences` (language, time zone, date/time/number formats, theme, document languages; JSON merge patch, `inherited`), PLT-18 `/v1.0/organization/preferences` (`organization.manage`); `IUserPreferences` in the SDK; notifications, recurrences and default OCR languages follow them | ✅ |
