...
| EVT-08 | Workflows | As an **Admin**, I want multi-step, long-running workflows (approvals, escalations, retention reviews), so that business processes run inside the system | Ext | P5 | #0009 |
...
| **P5 Collaboration, automation & integrations** | Share, automate, connect | PRV-01…03, PRV-05 (first), EVT-07…09 (rules and workflows), IAM-04, TAX-05, TAX-08…11, DOC-14, LST-17, NTF-06, API-03…06, API-08, API-09, PLT-06, DOC-05, DOC-06 (page operations, deferred from P3), DOC-15 (S3), NTF-04 (email, ntfy, Gotify) |
...
| **5b Automation** | Automation module ([ADR-0018](adr/0018-automation-workflowcore.md), [ADR-0019](adr/0019-workflows-on-wolverine.md), guide [automation.md](automation.md)). EVT-07 rules per workspace (`…/automation/rules`): trigger (`itemAdded`/`itemUpdated`/`itemDeleted` or an extension trigger; list, content type, changed fields), OData condition, ordered actions; runs once per event with a run log; chains stop at depth 3. EVT-08 workflows (`…/automation/workflows`, versioned, resumed through durable messages): approval (assignees, due date, escalation), delay, condition (approval outcome or OData), action steps; start from an item or a rule; `GET /v1.0/me/approvals`, decisions, runs with log and cancel. EVT-09 `AddAutomationAction`/`AddAutomationTrigger` for extensions (`IAutomationTriggers`). DOC-14 path templates: action `item.file` files and renames items from tokens (`{created:yyyy}/{counterparty}`). Built-in actions `item.update`, `item.file`, `task.create`, `notify`, `workflow.start`. Rules and workflows travel with workspace templates | ✅ |
